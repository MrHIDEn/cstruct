export class ModelParser {
    static _extractCKindFields(body, end = '') {
        return (body
            .match(/\w+\s[\w,]+;/g) ?? [])
    ...
    static dynamicStringOrArray(json) {
    ...
    static CKindStruct(json) {
        ...
        const matches = json.match(/struct\s(\w+):{(.*)};/g) ??
            json.match(/typedef\s+struct:{(.*)}(\w+);/g) ??
            json.match(/(\w+):?{([\w ,;]+)};?/g) ??
            [];
        for (const match of matches) {
            const m = match.match(/struct\s+(?<name>\w+)\s*{\s*(?<body>.*)}\s*;/) ??
    ...
    static CKindStaticAndDynamicArrayOrString(json) {
    ...
    static CKindStaticArrayOrString(json) {
        // Special-5
        // Special-2
        // `u8 [3];` => `["u8","u8","u8"]`
        // `s [3];` => `s3`
