...
export class CStructBE extends CStruct {
    ...
    read(buffer, offset = 0) {
        const reader = new ReadBE(this.modelClone, buffer, offset, this._aliases);
        return {
    ...
    write(buffer, struct, offset = 0) {
        const writer = new WriteBE(this.modelClone, struct, buffer, offset, this._aliases);
        return {
    ...
    make(struct) {
        const writer = new MakeBE(this.modelClone, struct, this._aliases);
        return {
            buffer: writer.toBuffer(),
            offset: writer.offset,
            size: writer.size,
