...
function splitClearTypes(types) {
    let x = types.replace(/{/g,"{\n").split(/[;\n]/g);
    let y = [];
    for (let i = 0; i < x.length; i++) {
        let row = x[i].trim();
        if (!row) continue; // remove empty
        let comm = row.indexOf("//"); // remove comment
        if(comm==0) continue;
        if(comm>0) row = row.slice(0,comm);
        y.push(row);
    }
    return y;
}
...
function parseTypes(types) {
    ...
    for(const x  of split) {
        if(x === '}') continue;
            ...
            const type = x.split(/ *{/g, 2)[0];
            y[type] = {};
            ref = y[type];
        } else {
            const m = x.match(/(\w+[\w\d]*)\s*([\w\d\s,\[\]]+)/);
            if (m === null || m.length!=3) throw Error(`Syntax error, '${x}'`);
            const [, t, r] = m;
            const split = r.split(/\s*,\s*/);// ','
            if(split.length < r.split(' ').length) throw Error(`Syntax error, missing ',' in '${x}'`);
            for (let x of split) {
                x = x.trim();
                const m = x.match(/^(\w[\w\d]*)\[(\d+)\]$/);
                if (m == null) {    // not array
                    ref[x] = t;
                } else {            // array
                    let [, n, s] = m;
                    console.log([n, s]);
                    ref[n] = Array(+s).fill(t);
                }
            }
        }
    }
...
function parseTypes1(types) {
    const split = splitClearTypes(types);
    const join = split.join(';').replace(/{;/g,"{") + ';';
    const m = join.match(/[\s\w]*\{[\w\d\s,;\[\]]+\}\s*;/g);
    types = {};
    // let tmp = types;
    // tmp = tmp.replace(/\n/g, '');
    // console.log(`"${tmp}"`);
