export class Read {
     ...
     */
    _readDynamicArray(keyArray, key, struct) {
        if (keyArray.slice(0, -6) !== key)
            throw SyntaxError(`A key 'aKey' must folow array declaration 'aKey.array'.`);
        // Read array length
     ...
     * <stringLength><string>
     */
    _readDynamicString(keyString, key, struct) {
        ...
        // Read string
        const stringValue = this._reader.read('str', stringLength);
    ...
    _read(struct, key, type) {
        switch (typeof type) {
            case 'object':
                ...
                break;
            case 'string':
                struct[key] = this._reader.read(type, struct[key]);
                break;
            default:
